...
if($format<1){
    ...
    }else{ ///SI NO ES FACTURA NORMAL CON FORMATO 1 
...
if($tPago=='Credito'){
        $iSQL="INSERT INTO cuenta_cobrar (id_Cliente,id_Comprobante,fecha_Inicio,fecha_Vence) 
                values($cli,$numFact,'$fec','$fecPlazo')";
        mysql_query($iSQL);
        if($tipoC=='Colones'){
            $sSQL="UPDATE cuenta_cobrar Set moneda='COLONES' Where id_Comprobante={$numFact}";
            mysql_query($sSQL);
        }else if($tipoC=='Dolares'){
            $sSQL="UPDATE cuenta_cobrar Set moneda='DOLARES' Where id_Comprobante={$numFact}";
            mysql_query($sSQL);
        }else{
            echo '<script language="javascript">alert("ERROR");</script>'; 
        }
...
//variable que guarda el nombre del archivo PDF
    ...
    $top_productos = 100;
        ...
        $pdf->Cell(30, 5, 'DESCRIPCION', 0, 1, 'C');
        $pdf->SetXY(80, $top_productos);
        $pdf->Cell(40, 5, 'PRESENTACION', 0, 1, 'C');
        $pdf->SetXY(110, $top_productos);
        $pdf->Cell(40, 5, 'PRECIO UNIDAD', 0, 1, 'C');    
    ...
    if($tPago='Credito'){
        ...
        if (mysql_query($recibo)){
            ...
            if($saldoG>0){
                $iSQL2="UPDATE cuenta_cobrar SET saldo_Global= {$saldoG}, saldo_global_dolares={$saldoGF}
                    WHERE id_cliente={$cli}";
                                
                mysql_query($iSQL2);

            }else if($saldoGF>0){
                $iSQL2="UPDATE cuenta_cobrar SET saldo_Global= {$saldoG}, saldo_global_dolares={$saldoFG}
                    WHERE id_cliente={$cli}";
