...
function generaConsultaRepProd($info){
	if($info['fechaD']=="" && $info['fechaH']==""){
								$consul="SELECT f.id ,f.fecha,pr.descripcion,dt.cantidad,p.price_out 
								from comprobante f,comprobante_detalle dt,product p,presentacion pr 
							...
							SELECT id from product where name='{$info['detalleP']}')
							and Producto_id=p.id  and STR_TO_DATE(fecha,'%d-%m-%Y') between 
							STR_TO_DATE('{$fD}','%d-%m-%Y') and	STR_TO_DATE('{$fH}','%d-%m-%Y')
							and dt.comprobante_id=f.id and p.presentation_id=pr.id
