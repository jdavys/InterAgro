...
if($tipo==4){
	...
	/*$respC="SELECT id from person where name='{$cliente}'";
	var_dump($respC);
	if (mysql_query($respC,$conexion)){
			
  		$cl = mysql_query($respC,$conexion);

		$cli = mysql_fetch_array($cl);
		var_dump($cli['id']);
		$clie=$cli[0];
	}*/
	...
	if (mysql_query($recibo,$conexion)){
		...
		$clie=$cuenta['id_Cliente'];
		
		if($monto <= $saldoFact && $monto>0){
			//var_dump($clie);
			$consulta="INSERT INTO abono (comprobante_id,cliente_id,monto,fecha,id_tipo_credito,detalle)
			values({$factura},{$clie},{$monto},'{$fecha}',{$tipo},'{$deta}')";
			var_dump($consulta);
			if (mysql_query($consulta,$conexion)){
					...
					if($tPago=='Credito'){
							if($mone=='DOLARES'){
									$saldoG=0;
									$global= "SELECT * from cuenta_cobrar where id_cliente= {$clie} and tipo_pago='Credito' and estado='PENDIENTE'and moneda='DOLARES'";
									if (mysql_query($global,$conexion)){
										...
										$iSQL2="UPDATE cuenta_cobrar SET saldo_global_dolares = {$saldoG}
							                		WHERE  id_cliente= {$clie}";
							            	
							            if(mysql_query($iSQL2,$conexion)){
							        		header("location: edit_abono.php?clie={$clie}");
											exit();
							       		}else{
							...
							if($mone=='COLONES'){
									$saldoG=0;
									$global= "SELECT * from cuenta_cobrar where id_cliente= {$clie} and tipo_pago='Credito' and estado='PENDIENTE'and moneda='COLONES'";
									if (mysql_query($global,$conexion)){
										$respuesta=mysql_query($global,$conexion);
